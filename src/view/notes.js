...
export default class Notes extends Component {
    ...
    async handleSubmission()  {
        ...
        await this.props.app.state.componentList.getOperationsFactory().componentDispatch({ [key+"note"]: this.state.value,}) 
...
render(){
        ...
        return(
            <div style= {{background:styles.colors.White1, padding:".5vw", borderRadius:".7vw",
              ...
              fontSize: styles.fonts.fontSubheader1}}>Note:</div></label>
<textarea onClick={()=>{this.setState({clicked:true})}} rows = {this.state.clicked? "4":"1"} 

style= {{fontFamily: styles.fonts.fontNormal, 
resize: "vertical", width:"13vw", marginTop:"1vh", height:"3vh",
              fontSize: styles.fonts.fontBody, marginBottom:".5vh"
            }} type="text" id="last"  value={this.state.value}  placeholder = {component?.getJson().note} onChange={
                this.handleChange} name={"note"}>
                    
                </textarea>
                            {/* <input onClick={()=>{this.setState({clicked:true})}} rows = {this.state.clicked? "4":"1"} style= {{fontFamily: styles.fonts.fontNormal,
